...
export type MigrationTarget = { url: string }
...
export function resolveMigrationDatabaseUrl(
  ...
  return directDatabaseUrl ?? runtimeDatabaseUrl
}
export function resolveMigrationTarget(databaseUrl: string | undefined): MigrationTarget {
...
async function trackedPostgresMigrations(client: PostgresClient): Promise<Map<string, string>> {
...
async function postgresSchemaState(client: PostgresClient): Promise<'empty' | 'existing'> {
...
async function applyPostgresBaseline(client: PostgresClient, baseline: MigrationFile, absorbed: MigrationFile[]): Promise<void> {
...
async function applyPostgresIncremental(client: PostgresClient, migration: MigrationFile): Promise<void> {
  await client.query('BEGIN')
  try {
    await client.query(migration.sql)
    await client.query(
      `INSERT INTO app_schema_migration (name, checksum) VALUES ($1, $2)`,
      [migration.name, migration.checksum],
    )
    await client.query('COMMIT')
  } catch (error) {
...
async function migratePostgres(url: string, migrationsDirectory: string): Promise<void> {
  ...
  try {
    ...
    if (!baseline) throw new Error(`Missing PostgreSQL baseline: ${POSTGRES_BASELINE}`)
    ...
    if (!tracked.has(POSTGRES_BASELINE)) {
      const absorbed = files.filter((file) => file.name.localeCompare(POSTGRES_BASELINE) <= 0)
    ...
    for (const file of files) {
      if (file.name.localeCompare(POSTGRES_BASELINE) <= 0 || tracked.has(file.name)) continue
...
export async function migrateDatabase(
  ...
  const { url } = resolveMigrationTarget(databaseUrl)
  await migratePostgres(url, migrationsDirectory)
}
