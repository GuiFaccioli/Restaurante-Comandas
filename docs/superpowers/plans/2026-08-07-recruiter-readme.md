# Recruiter README Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the terse technical README with a PT-BR product-and-engineering narrative suitable for public recruiter review.

**Architecture:** The README remains a single Markdown entry point. It will lead with the operational problem and solution, then progressively disclose product workflows, inventory intelligence, UX/UI practices, architecture, quality, roadmap, and local execution.

**Tech Stack:** Markdown, Next.js 16, React 19, TypeScript, Neon Postgres, Drizzle ORM, Neon Auth, Vitest, Playwright.

## Global Constraints

- Use PT-BR only.
- State only facts verified in the repository.
- Do not include credentials, personal data, or a public staging URL.
- Present the informative shopping list as roadmap, not as a delivered feature.
...
**Interfaces:**
- Consumes: verified stack and feature boundaries in `package.json`, `lib/stock/`, `lib/tenant-events.ts`, `components/shell/mobile-bottom-navigation.tsx`, and `tests/`.
- Produces: public project documentation that a recruiter can read without prior context.

- [ ] **Step 1: Verify the current README lacks the intended narrative**

Run: `rg -n "lista de compras|ficha técnica|experiência|qualidade" README.md`

Expected: no complete, recruiter-oriented description of stock intelligence, UX/UI, and quality.
...
- [ ] **Step 3: Verify factual boundaries and Markdown quality**

Run: `rg -n "lista de compras|Neon Auth|Vitest|Playwright|ficha técnica" README.md && git diff --check -- README.md`

Expected: the shopping list appears only as roadmap, the verified stack and inventory terms appear, and `git diff --check` has no output.

- [ ] **Step 4: Commit the public documentation**

```bash
git add README.md
git commit -m "docs: present product and engineering story"
```
