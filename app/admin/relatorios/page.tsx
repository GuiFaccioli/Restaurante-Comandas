...
import { categoria, fichaTecnicaItem, insumo, itemPedido, mesa, pedido, produto } from '@/lib/db/schema'
import { RelatoriosAdminClient } from './client'

export const dynamic = 'force-dynamic'

function money(value: number) { return new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(value) }
function quantity(value: string) { const number = Number(value); return Number.isInteger(number) ? String(number) : number.toFixed(3).replace(/0+$/, '').replace(/\.$/, '') }
...
export default async function RelatoriosAdminPage() {
  ...
  const [pedidos, itens, estoque, fichas] = await Promise.all([
    ...
    db.select({ nome: insumo.nome, atual: insumo.estoqueAtual, minimo: insumo.estoqueMinimo, unidade: insumo.unidadeBase }).from(insumo).where(eq(insumo.tenantId, tenantId)).orderBy(insumo.nome),
    db.select({ produto: produto.nome, insumo: insumo.nome, quantidade: fichaTecnicaItem.quantidade, unidade: insumo.unidadeBase }).from(fichaTecnicaItem).innerJoin(produto, eq(fichaTecnicaItem.produtoId, produto.id)).innerJoin(insumo, eq(fichaTecnicaItem.insumoId, insumo.id)).where(eq(fichaTecnicaItem.tenantId, tenantId)).orderBy(produto.nome),
  ])
  ...
  const data = {
    ...
    estoque: estoque.map((item) => ({ nome: item.nome, atual: quantity(item.atual), minimo: quantity(item.minimo), unidade: item.unidade, situacao: Number(item.atual) <= Number(item.minimo) ? 'Atenção' : 'OK' })),
    fichas: fichas.map((item) => ({ produto: item.produto, insumo: item.insumo, quantidade: quantity(item.quantidade), unidade: item.unidade })),
  }
