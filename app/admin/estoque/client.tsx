...
function ShoppingListView({ shoppingListItems }: { shoppingListItems: ShoppingListItem[] }) {
  ...
  const shoppingListText = orderedItems
    .map((item) => `${item.nome} - ${formatStockQuantity(item.quantidadeSugerida, item.unidade)}`)
    .join('\n')
