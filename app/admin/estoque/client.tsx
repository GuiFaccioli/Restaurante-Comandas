...
function ShoppingListView({ shoppingListItems }: { shoppingListItems: ShoppingListItem[] }) {
  ...
  async function completeItem(item: ShoppingListItem, quantity?: string) {
    ...
    try {
      ...
      router.refresh()
      toast.success('Item conclu?do.')
    } catch (error) {
      const message = error instanceof Error ? error.message : 'N?o foi poss?vel concluir o item.'
