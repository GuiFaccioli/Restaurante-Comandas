...
describe('cashier order control', () => {
  it('uses a tenant-scoped cashier query with item details, totals, and payment status', () => {
    ...
    expect(queries).toContain('leftJoin(mesa')
    expect(queries).toContain("canal: pedido.canal")
    expect(queries).toContain('enderecoSnapshot: pedido.enderecoSnapshot')
    expect(queries).toContain('taxaEntregaAplicada: pedido.taxaEntregaAplicada')
