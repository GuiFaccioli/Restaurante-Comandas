...
CREATE TABLE login_rate_limit (
  ...
  CONSTRAINT login_rate_limit_scope_check CHECK (scope IN ('email_ip', 'ip')),
  CONSTRAINT login_rate_limit_key_hash_check CHECK (key_hash ~ '^[0-9a-f]{64}$'),
  CONSTRAINT login_rate_limit_failure_count_check CHECK (failure_count >= 0)
);
